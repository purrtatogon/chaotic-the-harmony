    ...
    <ul className={styles.pwChecklist} aria-label="Password requirements">
      {checks.map((c) => (
        <li key={c.key} className={c.passed ? styles.pwCheckPassed : styles.pwCheckFailing}>
          <span aria-hidden="true" className={styles.pwCheckIcon}>
            {c.passed ? '✓' : '✕'}
          </span>
          <span>{c.label}</span>
        </li>
...
const SignUpPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!fullName.trim() || !email.trim() || pwError || confirmPassword !== password) return;
        ...
        <form onSubmit={handleSubmit} noValidate className={styles.authForm} aria-label="Create account form">
          ...
          <div className={styles.authField}>
            ...
            <p id="signup-password-hint" className={styles.authHint}>
              Must be at least {PASSWORD_MIN_LENGTH} characters with uppercase, lowercase, numbers, and special characters.
            </p>
            <input
              id="signup-password"
              type="password"
            ...
            />
            <PasswordChecklist password={password} visible={password.length > 0} />
            {errors.password && (
