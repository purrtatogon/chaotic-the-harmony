...
const MegaNav = () => {
  ...
  const { isAuthenticated, username } = useCustomerAuth();
  ...
  /** Collapse store mega-panel nesting when desktop flyout isn't "store". */
  const visibleStoreSection = openMenu === 'store' ? activeStoreSection : null;
  ...
  const activeSection = STORE_SECTIONS.find((s) => s.id === visibleStoreSection);
    ...
    >
      <ul role="list" className={styles.storePrimary}>
        ...
        {STORE_SECTIONS.map((section) => {
          const isActive = visibleStoreSection === section.id;
          return (
            <li key={section.id}>
              <button
      ...
      <div className={`${styles.storeSubPanel} ${activeSection ? styles.storeSubPanelVisible : ''}`}>
        {STORE_SECTIONS.map((section) => (
          <ul
            ...
            hidden={visibleStoreSection !== section.id}
        ...
        <div className={styles.navInner}>
          ...
          >
            ...
            <ul role="list" className={styles.navActions}>
              ...
              <li>
                <Link to="/profile" className={styles.navIconButton} aria-label="Wishlist" onClick={handleLinkClick}>
                  <HeartIcon />
                </Link>
              </li>
              <li>
                {isAuthenticated ? (
                  <Link to="/profile" className={styles.navLoginButton} aria-label="My profile" onClick={handleLinkClick}>
                    <UserIcon />
                    <span className={styles.navLoginLabel}>{username ? username.split(' ')[0] : 'Profile'}</span>
                  </Link>
                ) : (
                  <Link to="/login" className={styles.navLoginButton} aria-label="Account" onClick={handleLinkClick}>
                    <UserIcon />
                    <span className={styles.navLoginLabel}>Account</span>
                  </Link>
                )}
              </li>
              <li>
