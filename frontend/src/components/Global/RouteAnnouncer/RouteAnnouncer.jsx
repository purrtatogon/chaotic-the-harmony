...
function getPageName(pathname) {
  if (pathname === '/admin/users') {
    try {
      const r = (localStorage.getItem('admin_userRole') || '').replace(/^ROLE_/i, '');
      return r === 'ADMIN' ? 'Users' : 'Customers';
    } catch {
      return 'Accounts';
    }
  }
  ...
  if (/^\/admin\/site-content\/[^/]+$/.test(pathname)) {
    return 'Edit site content';
  }
...
const RouteAnnouncer = () => {
  ...
  useLayoutEffect(() => {
    ...
    /* Clear then set so repeated navigation to the same route re-announces (state updates defer to rAF) */
    let rafOuter = 0;
    let rafInner = 0;
